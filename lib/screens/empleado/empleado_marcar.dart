...
class _EmpleadoMarcarState extends State<EmpleadoMarcar> {
  ...
  Widget _recientes() {
    final registros = AsistenciaService.instance.historialLocal();
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          for (final r in ultimos)
            Padding(
              ...
              child: Row(children: [
                Icon(r['tipo'] == 'entrada' ? Icons.login : Icons.logout,
                    size: 18,
                    color: r['tipo'] == 'entrada'
                        ? AppColors.success
                        : AppColors.textSoft),
                const SizedBox(width: 10),
                Expanded(
                  child: Column(
                      ...
                      children: [
                        Text(
                            '${r['tipo'] == 'entrada' ? 'Entrada' : 'Salida'} · ${r['obra_nombre'] ?? ''}',
                            style: const TextStyle(
                                fontSize: 13,
                                fontWeight: FontWeight.w500,
                                color: AppColors.textDark)),
                        Text(_fechaHora(r['hora'] as String?),
                            style: const TextStyle(
                                fontSize: 11, color: AppColors.textMuted)),
                      ]),
                ),
                if (r['distancia_metros'] != null)
                  Text('${r['distancia_metros']} m',
                      style: const TextStyle(
                          fontSize: 11, color: AppColors.textMuted)),
              ]),
            ),
